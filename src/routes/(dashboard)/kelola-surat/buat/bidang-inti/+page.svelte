<script>
    ...
    async function onSave(data) {
        try {
            ...
            alert(
                `Surat Bidang Inti Tujuan: ${data.kepada}\nPerihal: ${data.perihal}\nBerhasil Disimpan!`,
            );
            goto("/kelola-surat");
        } catch (error) {
...
<svelte:head>
    <title>Surat Bidang Inti - SiSurat</title>
</svelte:head>
...
<div class="space-y-6 max-w-5xl mx-auto">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-bold text-[var(--dark)] tracking-tight">
                Buat Surat Keluar (Bidang Inti)
            </h1>
            <p class="text-slate-500 mt-1">
                Lengkapi rancangan data surat resmi kelembagaan pengurus inti di
                bawah ini.
            </p>
        </div>
    </div>
    ...
    <!-- Form -->
    <SuratBidangIntiForm {onSave} {onCancel} {isSaving} />
</div>
